document.addEventListener("DOMContentLoaded", function () {
    ...
    addToCartButtons.forEach(button => {      //loops through every "add to cart" button
        button.addEventListener("click", function (event) {
            ...
            //The closest() method finds the nearest ancestor that matches the given selector. When click add to cart, gets details of the specific product related to that button.
            const productName = productBox.querySelector("h3").innerText; //what ever h3 is called will be stored as a variable in productName
            const productPrice = parseFloat(productBox.querySelector("span").innerText.replace("£", "")); //parsefloat converts price from a string into number which can then be used for calculations. 
            const productImage = productBox.querySelector("img").src; //img is shown for the product selected when on cart page.

            addToCart(productName, productPrice, productImage); //when add to cart button is clicked on cart page the img, product name and price will be shown 
        });
    });
...
function changeQuantity(name, change) {  //name of the item you want to change (passed when the button is clicked) and amount to increase or decrease the quantity by.
    ...
    if (item) {
        ...
        localStorage.setItem("cart", JSON.stringify(cart)); //turn cart back into a string and is saves updated cart in localstorage.
        updateCartDisplay();
    }
}
...
// Ensure cart icon is updated after adding an item
function addToCart(name, price, image) {
    let cart = JSON.parse(localStorage.getItem("cart")) || [];
